        ...
        <div className="flex justify-between items-center ">
          ...
          <ul className="gap-3 hidden md:flex">
            {links.map((link, index) => (
              <li
                key={index}
                className="px-4 py-2 rounded-full transition-all duration-150 ease-in-out hover:text-red-500"
              >
          ...
          <ul
            className={`md:hidden absolute top-20 z-[40]  flex-col gap-3 bg-[#040404] p-4 transition-all duration-300 ease-in-out right-0 ${!isOpened ? "hidden" : "flex"}`}
          >
            {links.map((link, index) => (
              <a
                href={link.link}
                key={index}
                className="text-right px-4 py-2 rounded-full transition-all duration-150 ease-in-out hover:text-red-500"
              >
                {link.name}
              </a>
            ))}
            <a
              href="/#contacts"
              className="px-6 py-2 font-semibold text-white uppercase bg-red-500 rounded-full transition-all duration-150 ease-in-out hover:bg-red-600"
            >
              Book Now
            </a>
          </ul>
